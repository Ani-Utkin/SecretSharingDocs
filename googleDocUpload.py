...
def DownloadFile(id, title, mime):

    request = drive_service.files().export_media(fileId=id, mimeType=mime)
    fh = io.BytesIO()
    ...
    downtitle = title.get('title') + ".docx"
...
def SSSReconstruct(docIDs):
    ...
    reconDrive = drive_service.files().copy(fileId=docIDs[0], body=reconstruct_title).execute()
    ...
    DownloadFile(reconDrive.get('id'), reconstruct_title, 'application/vnd.openxmlformats-officedocument.wordprocessingml.document')


def DocsAPI(DOCUMENT_ID):
    global id_list
    ...
    mime_file={'mimeType' : 'application/vnd.google-apps.document'}
    getFile = drive_service.files().copy(fileId=DOCUMENT_ID, body=mime_file).execute()
    file = doc_service.documents().get(documentId=getFile.get('id')).execute()
