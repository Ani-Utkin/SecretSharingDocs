...
def ReconstructSheets(shareSheetsIDs):
    ...
    DownloadFile(recon_spreadsheet.get('spreadsheetId'), 'Reconstructed Document', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')


def SheetsAPI(SHEETS_ID):
    ...
    getSheet = drive_service.files().copy(fileId=SHEETS_ID, body={"mimeType": "application/vnd.google-apps.spreadsheet"}).execute()
    sheet = sheets_service.spreadsheets().values().get(spreadsheetId=getSheet.get('id'), range='Sheet1').execute()
